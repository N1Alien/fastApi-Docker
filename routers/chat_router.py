...
async def create_chat_session(current_user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    ...
    try:
        # Usuwamy ewentualne śmieci z user_id i rzutujemy na czysty int
        clean_uid = int(float(current_user_id.replace('(', '').replace(')', '').replace(',', '').strip()))
        ...
        row = result.fetchone()
        # POPRAWKA GŁÓWNA: Jawnie wyciągamy indeks 0 z krotki bazy Neon (czysta cyfra)
        clean_session_id = int(row[0]) if row else None
...
async def chat_with_model(request_data: ChatRequestSchema, current_user_id: str = Depends(get_current_user_id)):
    ...
    # Czyszczenie zanieczyszczonego formatu user_id z tokenu przed wejściem do generatora grafu
    clean_user_string = request_data.prompt
    raw_uid = current_user_id.replace('(', '').replace(')', '').replace(',', '').strip()
    
    return StreamingResponse(
        agent_stream_generator(prompt=request_data.prompt, session_id=request_data.session_id, user_id=str(raw_uid)), 
